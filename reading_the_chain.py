...
def connect_to_eth():
	...
	url = "https://eth-mainnet.g.alchemy.com/v2/WhX6fKzlOpjrJHeXbsx6Eg1cLfrjHIj3"
	w3 = Web3(HTTPProvider(url))
	assert	w3.is_connected(), f"Failed to connect"
	return w3
...
def is_ordered_block(w3, block_num):
	...
	for transaction in block.transactions:
		priority_fee = 0
		if base_fee is None:
			priority_fee = transaction.get('gasPrice')
		else:
			maxFeePerGas = transaction.get('maxFeePerGas', None)
			maxPriorityFeePerGas = transaction.get('maxPriorityFeePerGas', None)
			if maxFeePerGas and maxPriorityFeePerGas:
				priority_fee = min(maxPriorityFeePerGas, maxFeePerGas - base_fee)
			else:
				priority_fee = transaction.get('gasPrice') - base_fee
		
		if prev_fee is not None and priority_fee > prev_fee:
			return False
		
		prev_fee = priority_fee
